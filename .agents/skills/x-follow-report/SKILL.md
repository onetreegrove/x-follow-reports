...
使用此 skill 抓取用户的 X 关注流或主页时间线。脚本只采集并导出结构化素材：默认向 stdout 打印素材 JSON；传入 `--materials-file` 时写入指定文件。当前执行环境根据素材直接撰写中文 AI 开发者早/午/晚报，并按照日期结构保存到项目根目录下的 .x-follow-report/report-outputs 目录。脚本不生成兜底 Markdown 报告。
...
bun install
bun main.ts --timeline following --max-items 200 --lookback-hours 24 --materials-file /tmp/x-follow-materials.json
```
...
4. **报告撰写**：当前执行环境读取素材，按下方“报告模板”生成中文 AI 开发者早/午/晚报，包含今日要点、资讯标题、正文和关联推文。
...
### 2. 存储规范 (重要)
- **存储位置**：项目根目录下的 `.x-follow-report/report-outputs/` 目录。
- **目录结构**：`.x-follow-report/report-outputs/YYYY-MM/DD/HHMMSS-早|午|晚报.md`
- **写入方式**：使用 `write_file` 写入。**严禁**生成 `x-follow-reports/` 目录或逐条推文的 `.md` 文件。

### 3. 报告模板

最终 Markdown 必须使用以下结构：
...
模板要求：
- 顶部字段只保留 `生成时间`、`来源`。
- `今日要点` 使用 3-6 条项目符号，概括当天最重要的产品、模型、工具、生态或行业变化。
- `资讯` 下每条动态使用三级标题 `###`，标题必须包含具体主体和动作。
- 每条资讯正文后保留 `相关推文：` 和来源链接；如果多条推文属于同一事件，合并在同一个资讯条目下。
- 不使用额外的二级栏目分组；除 `今日要点` 和 `资讯` 外，不新增其他 `##` 标题。
