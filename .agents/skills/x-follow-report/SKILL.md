...
description: 抓取用户的 X/Twitter 关注流或主页时间线，使用非官方 X Web/API 凭据采集内容，并直接生成 AI 开发者早/午/晚报。当用户要求抓取、总结、分析、监控或报告 X 关注列表消息流、Following feed、Home timeline 时使用。
---
...
使用此 skill 抓取用户的 X 关注流或主页时间线。推荐工作流是脚本采集并导出结构化素材；当前执行环境根据素材直接撰写中文 AI 开发者早/午/晚报，并按照日期结构保存到项目根目录。脚本在不传 `--materials-json` / `--materials-file` 时也可生成基础模板报告，主要用于调试或兜底。
...
- `include_replies`
- `include_reposts`
- `download_media`：兼容旧配置；素材始终记录媒体 URL，脚本不会下载二进制媒体。
- `report_language`
...
- 脚本推荐只导出素材，报告正文由当前执行环境撰写；默认 Markdown 输出仅作为基础模板报告兜底。
